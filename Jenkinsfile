    ...
    stage('Test') {
      agent {
        docker {
          ...
          args '-v $HOME/jenkins:/usr/src -e SONAR_HOST_URL="http://http://44.242.146.190//" -e SONAR_LOGIN="a8fd5149bbd092aabe4af5656283ce9154bf34f2"'
        }
...
sonar-scanner -Dsonar.projectKey=dott -Dsonar.sources=. -Dsonar.host.url="$SONAR_HOST_URL" -Dsonar.login="$SONAR_LOGIN" -X
'''
        }
